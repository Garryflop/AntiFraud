# 🛡️ AntiFraud Monitoring System (ФСМС / ДЭР по области Абай)
...
## 🏗️ Архитектура Шлюзового Фильтра (MIS Proxy Architecture)
...
│     (Damumed, KazMedInfo, КМИС, Региональные МИС РК)        │
└──────────────────────────────┬──────────────────────────────┘
                               │  Передача реестров оказанных услуг
                               ▼  (REST API Protocol)
┌─────────────────────────────────────────────────────────────┐
│              🛡️ АНТИФРОД ШЛЮЗ-ФИЛЬТР (ФСМС)                  │
│                                                             │
│  [1] Hard Rules Engine    (ЗАГС, КНБ, График отпусков)      │
│  [2] Hybrid ML Engine     (IsoForest + RandomForest ML)     │
│  [3] Real-time Audit Trail (Интерактивная аналитика ДЭР)    │
└──────────────────────────────┬──────────────────────────────┘
                               │
            ┌──────────────────┴──────────────────┐
            ▼                                     ▼
   [ Одобрено (95%) ]                    [ Выявлены нарушения ]
 💳 Выплата средств ФСМС               🛑 Авто-приостановка выплат 
                                       📊 Отправка материалов в ДЭР
```
...
## 🛠️ Технологический стек и Структура проекта
